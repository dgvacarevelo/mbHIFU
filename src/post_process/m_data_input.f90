...
module m_data_input
    ...
    abstract interface
        ...
        impure subroutine s_read_abstract_data_files(t_step, hifu_id)
    ...
    type(scalar_field), allocatable, dimension(:), public    :: q_cons_hifu  !< HIFU variables
...
contains
    ...
    impure subroutine s_setup_mpi_io_params(data_size, m_MOK, n_MOK, p_MOK, WP_MOK, MOK, str_MOK, NVARS_MOK, hifu_comm)
        ...
        if (hifu_comm) then
            ...
            if (hifu_params%stg3 .and. hifu_params%cartesian) alt_sys = hifu_idx%qth
        else
            alt_sys = sys_size
        end if
    ...
    impure subroutine s_allocate_field_arrays(local_start_idx, end_x, end_y, end_z)
        ...
        if (hifu) then
            do i = 1, sys_size_hifu
                allocate (q_cons_hifu(i)%sf(local_start_idx:end_x,local_start_idx:end_y,local_start_idx:end_z))
            end do
        end if
    ...
    !! variables.
    impure subroutine s_read_serial_data_files(t_step, hifu_id)
    ...
    impure subroutine s_read_parallel_data_files(t_step, hifu_id)
        ...
        hifu_comm = .false.
        if (present(hifu_id)) hifu_comm = .true.
        ...
        call s_read_parallel_conservative_data(t_step, m_MOK, n_MOK, p_MOK, WP_MOK, MOK, str_MOK, NVARS_MOK, hifu_comm)
    ...
    impure subroutine s_read_parallel_conservative_data(t_step, m_MOK, n_MOK, p_MOK, WP_MOK, MOK, str_MOK, NVARS_MOK, hifu_comm)
        ...
        if (file_per_process) then
            ...
            if (file_exist) then
                ...
                call s_setup_mpi_io_params(data_size, m_MOK, n_MOK, p_MOK, WP_MOK, MOK, str_MOK, NVARS_MOK, hifu_comm)
                ...
                if (hifu_comm) then
                    do i = 1, sys_size_hifu
                        ...
                        call MPI_FILE_SET_VIEW(ifile, disp, mpi_p, MPI_IO_HIFU_DATA%view(i), 'native', mpi_info_int, ierr)
                        call MPI_FILE_READ_ALL(ifile, MPI_IO_HIFU_DATA%var(i)%sf, data_size, mpi_io_p, status, ierr)
                    end do
                else
                    do i = 1, sys_size
                        var_MOK = int(i, MPI_OFFSET_KIND)
