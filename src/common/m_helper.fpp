...
contains
    ...
    impure subroutine s_initialize_bubble_vars()
        ...
        if (bubbles_lagrange) then
            ...
            Tw = bub_pp%Thost/T0
            pv = pv/p0
            k_vl = bub_pp%k_v*(T0/(x0*rho0*c0*c0*c0))
            k_gl = bub_pp%k_g*(T0/(x0*rho0*c0*c0*c0))
            cp_v = bub_pp%cp_v*(T0/(c0*c0))
            cp_g = bub_pp%cp_g*(T0/(c0*c0))
            R_v = (bub_pp%R_v)*(T0/(c0*c0))
            R_g = (bub_pp%R_g)*(T0/(c0*c0))
            vd = vd/(x0*c0)
            ss = ss/(rho0*x0*c0*c0)
