...
module m_boundary_common
    ...
    private; public :: s_initialize_boundary_common_module, s_populate_variables_buffers, s_populate_capillary_buffers, &
        & s_populate_F_igr_buffers, s_populate_grid_variables_buffers, s_finalize_boundary_common_module, s_populate_EL_buffers
...
contains
    ...
    impure subroutine s_populate_bc_direction(bc_dir, bc_loc, bc_bounds, bc_type_edge, q_prim_vf, pb_in, mv_in, q_T_sf)
        ...
        $:GPU_PARALLEL_LOOP(private='[l, k, bc_code]', collapse=2, copyin='[mytime]')
        do l = l_beg, l_end
            do k = k_beg, k_end
                if (bc_dir == 1) then
                ...
                case (BC_ACOUSTIC_WAVE)
                    call s_acoustic_bc(q_prim_vf, bc_dir, bc_loc, k, l, mytime)
                case (BC_AXIS_SECTOR)
                    if (bc_dir == 2 .and. bc_loc == -1) call s_axis_sector_hifu(q_prim_vf, k, l)
                end select
