...
contains
    ...
    impure subroutine s_tvd_rk(t_step, time_avg, nstage)
        ...
        if (adap_dt) call s_adaptive_dt_bubble(1, t_step)
        ...
        if (adap_dt) call s_adaptive_dt_bubble(3, t_step)
    ...
    impure subroutine s_adaptive_dt_bubble(stage, t_step)

        integer, intent(in) :: stage, t_step
        ...
        if (bubbles_euler) then
            ...
            if (lag_params%initial_corrector .and. stage == 1) call s_initial_pressure_correction(q_prim_vf, bc_type)
            ...
            if (stage == 3) then
                ...
                if (lag_params%write_bubbles) then
                    call s_write_lag_particles(mytime + dt, replace=.false.)
                end if
                call s_write_void_evol(mytime + dt, .false.)
            end if
        end if
