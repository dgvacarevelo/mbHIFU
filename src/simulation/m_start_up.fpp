...
contains
    ...
    impure subroutine s_read_data_files(q_cons_vf, q_hifu_vf)
        ...
        if (.not. parallel_io) then
            call s_read_serial_data_files(q_cons_vf, q_hifu_vf)
        else
            call s_read_parallel_data_files(q_cons_vf, q_hifu_vf)
        end if
            ...
            if (file_exist) then
                ...
                if (bubbles_euler .or. elasticity) then
                    ...
                    do i = 1, sys_size_hifu
                        ...
                        call MPI_FILE_SET_VIEW(ifile, disp, MPI_DOUBLE_PRECISION, MPI_IO_HIFU_DATA%view(i), 'native', &
                                               & mpi_info_int, ierr)
                        call MPI_FILE_READ(ifile, MPI_IO_HIFU_DATA%var(i)%sf, data_size, MPI_DOUBLE_PRECISION, status, ierr)
                    end do
                else
                    do i = 1, sys_size
                        var_MOK = int(i, MPI_OFFSET_KIND)
    ...
    impure subroutine s_perform_time_step(t_step, time_avg)
        ...
        if (probe_wrt) then
            if (hifu_params%heatSolver) then
                $:GPU_UPDATE(host='[q_hifu%vf(hifu_idx%T)%sf]')
            else
                do i = 1, sys_size
    ...
    impure subroutine s_save_data(t_step, start, finish, io_time_avg, nt)
        ...
        if (bubbles_lagrange .and. .not. hifu_params%heatSolver) then
            $:GPU_UPDATE(host='[lag_id, mtn_pos, mtn_posPrev, mtn_vel, intfc_rad, intfc_vel, bub_R0, Rmax_stats, Rmin_stats, &
                         & bub_dphidt, gas_p, gas_mv, gas_mg, gas_betaT, gas_betaC, bub_qvis, bub_qth, bub_hifu_rad]')
            do i = 1, nBubs
            ...
            call s_write_data_files(q_cons_ts(stor)%vf, q_T_sf, q_prim_vf, save_count, bc_type, q_beta%vf(1))
            call s_write_restart_lag_bubbles(save_count)  ! parallel
            if (lag_params%write_bubbles_stats) call s_write_lag_bubble_stats()
        else if (.not. hifu_params%heatSolver) then
