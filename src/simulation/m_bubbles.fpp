...
contains
    ...
    elemental function f_rddot(fRho, fP, fR, fV, fR0, fpb, fpbdot, alf, fntait, fBtait, f_bub_adv_src, f_divu, fCson, fInt, &
                               & fshell, fRbuck, fRcell)

        $:GPU_ROUTINE(parallelism='[seq]')
        real(wp), intent(in) :: fRho, fP, fR, fV, fR0, fpb, fpbdot, alf
        real(wp), intent(in) :: fntait, fBtait, f_bub_adv_src, f_divu
        real(wp), intent(in) :: fCson, fInt, fshell, fRbuck, fRcell
        real(wp)             :: fCpbw, fCpinf, fCpinf_dot, fH, fHdot, c_gas, c_liquid, p_out
        real(wp)             :: f_rddot
        ...
        if (bubble_model == bubble_model_gilmore) then
            ...
            if (bubbles_euler) then
                ...
                if (lag_params%pressure_corrector .and. any(lag_params%interaction_model == (/1, 3/)) .and. adap_dt) then
                    p_out = f_pout(fpbdot, fP, fCpbw, fRho, fR, fV, fshell, fRcell)
                    fCpinf = fCpinf + p_out
                end if
                c_liquid = fCson
            end if
    ...
    elemental function f_cpbw_KM(fR0, fR, fV, fpb, fshell, fRbuck)

        $:GPU_ROUTINE(parallelism='[seq]')
        real(wp), intent(in)           :: fR0, fR, fV, fpb
        real(wp), intent(in), optional :: fshell, fRbuck
        real(wp)                       :: f_cpbw_KM, ss_mod
        ...
        if (bubbles_lagrange .and. fshell == 1._wp) then
            ss_mod = 0._wp
            if (fR > fRbuck) ss_mod = el_vsc*((fR/fRbuck)**2._wp - 1._wp)
            f_cpbw_KM = f_cpbw_KM - 2._wp*ss_mod/fR - 4._wp*dil_vsc*fV/(fR**2._wp)
        else
    ...
    elemental function f_rddot_KM(fpbdot, fCp, fCpbw, fRho, fR, fV, fR0, fC, fInt, fshell, fRbuck, fpb)

        $:GPU_ROUTINE(parallelism='[seq]')
        real(wp), intent(in) :: fpbdot, fCp, fCpbw
        real(wp), intent(in) :: fRho, fR, fV, fR0, fC
        real(wp), intent(in) :: fInt, fshell, fRbuck, fpb
        real(wp)             :: denom, tmp1, tmp2, cdot_star, ss_mod
        real(wp)             :: f_rddot_KM
        ...
        if (bubbles_lagrange .and. fshell == 1._wp) then
            ...
            if (fR > fRbuck) then  ! Elastic regime
                ss_mod = el_vsc*((fR/fRbuck)**2._wp - 1._wp)
                cdot_star = cdot_star - 4._wp*el_vsc*fV/fRbuck**2._wp
            end if
            denom = denom + 4._wp*dil_vsc/(fRho*fC*fR)
            cdot_star = cdot_star + 2._wp*ss_mod*fV/(fR**2._wp) + 8._wp*dil_vsc*(fV**2._wp)/fR**3._wp
        else
    ...
    elemental subroutine s_vflux(fR, fV, fpb, fmass_v, iR0, vflux, fmass_g, fbeta_c, fR_m, fgamma_m, fshell)
        ...
        real(wp), intent(in), optional  :: fshell
    ...
    elemental function f_bpres_dot(fvflux, fR, fV, fpb, fmass_v, iR0, fbeta_t, fR_m, fgamma_m, fshell)
    ...
    !! @param fpbdot Time-derivative of internal bubble pressure @param fCp Driving pressure @param fCpbw Bubble wall pressure
    !! @param fRho Current density @param fR Current bubble radius @param fV Current bubble velocity @param fR0 Equilibrium bubble
    !! radius @param fC Current sound speed
        ...
        do
            ...
            do while (iter_count < adap_dt_max_iters)
                ...
                call s_advance_substep(err(1), fRho, fP, fR, fV, fR0, fpb, fpbdot, alf, fntait, fBtait, f_bub_adv_src, f_divu, &
                                       & bub_id, fmass_v, fmass_g, fbeta_c, fbeta_t, fnoise_constant, flambda_c, fdk, floc, &
                                       & ftime + t_new, fAc1, fCson, fInt, fshell, fRbuck, fRcell, h, myR_tmp1, myV_tmp1, &
                                       & myPb_tmp1, myMv_tmp1)
                ...
                call s_advance_substep(err(2), fRho, fP, fR, fV, fR0, fpb, fpbdot, alf, fntait, fBtait, f_bub_adv_src, f_divu, &
                                       & bub_id, fmass_v, fmass_g, fbeta_c, fbeta_t, fnoise_constant, flambda_c, fdk, floc, &
                                       & ftime + t_new, fAc21, fCson, fInt, fshell, fRbuck, fRcell, 0.5_wp*h, myR_tmp2, myV_tmp2, &
                                       & myPb_tmp2, myMv_tmp2)
                ...
                call s_advance_substep(err(3), fRho, fP, fR2, fV2, fR0, fpb2, fpbdot, alf, fntait, fBtait, f_bub_adv_src, f_divu, &
                                       & bub_id, fmass_v2, fmass_g, fbeta_c, fbeta_t, fnoise_constant, flambda_c, fdk, floc, &
                                       & ftime + t_new + 0.5_wp*h, fAc22, fCson, fInt, fshell, fRbuck, fRcell, 0.5_wp*h, &
                ...
                if ((err(1) <= adap_dt_tol) .and. (err(2) <= adap_dt_tol) .and. (err(3) <= adap_dt_tol) .and. (err(4) &
                    ...
                    ! if (bub_id == 50) then print*, 'ss:', fR, fV, fAc, fAc21, fAc22, t_new end if
                    ...
                    if (bubbles_lagrange) then
                        ...
                        if (hifu_params%sampling) then
                            ...
                            fvis_inst = (4._wp*pi*fR**2._wp)*(4._wp*mu_l*(fV**2._wp)/(fR))
                    ...
                    ! if (iter_count >= adap_dt_max_iters) then print *, 'h small', h, t_new, bub_id, fR, fV, fshell print *,
                    ! 'errs', err(1), err(2), err(3), err(4), err(5) print *, 'tmp R', myR_tmp1(1), myR_tmp1(2), myR_tmp1(3),
                    ! myR_tmp1(4) print *, 'tmp V', myV_tmp1(1), myV_tmp1(2), myV_tmp1(3), myV_tmp1(4) print *, 'tmp Pb',
                    ! myPb_tmp1(1), myPb_tmp1(2), myPb_tmp1(3), myPb_tmp1(4) print *, 'tmp mass_v', myMv_tmp1(1), myMv_tmp1(2),
                    ! myMv_tmp1(3), myMv_tmp1(4) print *, 'old vals', fR, fV, fpb, fmass_v, fP, t_new, t_new/(0.5_wp*dt) print *,
                    ! 'otherVars', fmass_g, fbeta_c, fbeta_t, fCson, fshell, fRbuck, fRrupt end if
                end if
            end do
    ...
    subroutine s_initial_substep_h(fRho, fP, fR, fV, fR0, fpb, fpbdot, alf, fntait, fBtait, f_bub_adv_src, f_divu, fCson, fInt, &
                                   & fshell, fRbuck, fRcell, h)
        ...
        myR_tmp(1) = fR
        myV_tmp(1) = fV
        myA_tmp(1) = f_rddot(fRho, fP, myR_tmp(1), myV_tmp(1), fR0, fpb, fpbdot, alf, fntait, fBtait, f_bub_adv_src, f_divu, &
        ...
        myA_tmp(2) = f_rddot(fRho, fP, myR_tmp(2), myV_tmp(2), fR0, fpb, fpbdot, alf, fntait, fBtait, f_bub_adv_src, f_divu, &
                & fCson, fInt, fshell, fRbuck, fRcell)
    ...
    subroutine s_advance_substep(err, fRho, fP, fR, fV, fR0, fpb, fpbdot, alf, fntait, fBtait, f_bub_adv_src, f_divu, bub_id, &
        ...
        & fmass_v, fmass_g, fbeta_c, fbeta_t, fnoise_constant, flambda_c, fdk, floc, ftime, fAc, fCson, fInt, fshell, fRbuck, &
            & fRcell, h, myR_tmp, myV_tmp, myPb_tmp, myMv_tmp)
        ...
        real(wp), intent(in)                :: fshell, fRbuck, fInt, fRcell, fnoise_constant, flambda_c, fdk, floc, ftime
        ...
        if (bubbles_lagrange) then
            ...
            call s_advance_EL(myR_tmp(2), myV_tmp(2), myPb_tmp(2), myMv_tmp(2), bub_id, fmass_g, fbeta_c, fbeta_t, &
                              & mydPbdt_tmp(2), mydMvdt_tmp(2), fshell)
        end if
        ...
        if (bubbles_lagrange) then
            ...
            call s_advance_EL(myR_tmp(3), myV_tmp(3), myPb_tmp(3), myMv_tmp(3), bub_id, fmass_g, fbeta_c, fbeta_t, &
                              & mydPbdt_tmp(3), mydMvdt_tmp(3), fshell)
        end if
        ...
        if (bubbles_lagrange) then
            ...
            call s_advance_EL(myR_tmp(4), myV_tmp(4), myPb_tmp(4), myMv_tmp(4), bub_id, fmass_g, fbeta_c, fbeta_t, &
                              & mydPbdt_tmp(4), mydMvdt_tmp(4), fshell)
        end if
    ...
    elemental subroutine s_advance_EL(fR_tmp, fV_tmp, fPb_tmp, fMv_tmp, bub_id, fmass_g, fbeta_c, fbeta_t, fdPbdt_tmp, &
                                      & advance_EL, fshell)

        $:GPU_ROUTINE(parallelism='[seq]')
        real(wp), intent(in)    :: fR_tmp, fV_tmp, fPb_tmp, fMv_tmp
        real(wp), intent(in)    :: fmass_g, fbeta_c, fbeta_t, fshell
        integer, intent(in)     :: bub_id
        real(wp), intent(inout) :: fdPbdt_tmp
        real(wp), intent(out)   :: advance_EL
        real(wp)                :: fVapFlux, myR_m, mygamma_m

        call s_vflux(fR_tmp, fV_tmp, fPb_tmp, fMv_tmp, bub_id, fVapFlux, fmass_g, fbeta_c, myR_m, mygamma_m, fshell)
