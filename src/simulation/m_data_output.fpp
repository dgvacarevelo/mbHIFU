...
contains
    ...
    impure subroutine s_write_parallel_data_files(q_cons_vf, t_step, bc_type, beta, q_hifu_vf, q_T_sf)
        ...
        if (file_per_process) then
            ...
            if (bubbles_euler) then
                ...
                do i = 1, alt_sys
                    ...
                    call MPI_FILE_SET_VIEW(ifile, disp, MPI_DOUBLE_PRECISION, MPI_IO_HIFU_DATA%view(i), 'native', mpi_info_int, &
                                           & ierr)
                    if (hifu_params%cartesian .and. hifu_params%heatSolver) then
                        call MPI_FILE_WRITE(ifile, MPI_IO_HIFU_DATA%var(i)%sf, data_size, MPI_DOUBLE_PRECISION, status, ierr)
                    else
                        call MPI_FILE_WRITE_ALL(ifile, MPI_IO_HIFU_DATA%var(i)%sf, data_size, MPI_DOUBLE_PRECISION, status, ierr)
                    end if
                end do
            else
                do i = 1, sys_size  ! TODO: check if sys_size is correct
    ...
    impure subroutine s_write_probe_files(t_step, q_cons_vf, accel_mag, q_hifu_vf)
        ...
        do i = 1, num_probes
                    ...
                    if ((probe(i)%x >= x_cb(-1)) .and. (probe(i)%x <= x_cb(m))) then
                        if ((probe(i)%y >= y_cb(-1)) .and. (probe(i)%y <= y_cb(n))) then
                            ...
                            if (elasticity) then
                                ...
                                call s_compute_pressure(q_cons_vf(1)%sf(j - 2, k - 2, l), q_cons_vf(eqn_idx%alf)%sf(j - 2, k - 2, &
                                ...
                                call s_compute_pressure(q_cons_vf(eqn_idx%E)%sf(j - 2, k - 2, l), &
                                                        & q_cons_vf(eqn_idx%alf)%sf(j - 2, k - 2, l), dyn_p, pi_inf, gamma, rho, &
                                                        & qv, rhoYks, pres, T)
                            end if
                            ...
                            if ((probe(i)%z >= z_cb(-1)) .and. (probe(i)%z <= z_cb(p))) then
                                ...
                                if (hifu_params%heatSolver) then
                                    ...
                                    if (elasticity) then
                                        ...
                                        call s_compute_pressure(q_cons_vf(1)%sf(j - 2, k - 2, l - 2), &
                                                                & q_cons_vf(eqn_idx%alf)%sf(j - 2, k - 2, l - 2), dyn_p, pi_inf, &
                                                                & gamma, rho, qv, rhoYks, pres, T, &
                                    ...
                                    ! Compute mixture sound speed
                                    call s_compute_speed_of_sound(pres, rho, gamma, pi_inf, ((gamma + 1._wp)*pres + pi_inf)/rho, &
            ...
            if (proc_rank == 0) then
                if (hifu_params%cartesian .and. hifu_params%heatSolver) then
                    write (i + 30, '(6X,6e24.8)') nondim_time, rho, vel(1), vel(2), pres, Temp_hifu
                else if (n == 0) then
                    ...
                    if (hifu) then
                        #:if not MFC_CASE_OPTIMIZATION or num_dims > 1
                            write (i + 30, '(6X,6F24.8)') nondim_time, rho, vel(1), vel(2), pres, Temp_hifu
                        #:endif
