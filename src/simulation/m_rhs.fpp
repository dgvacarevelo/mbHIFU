...
contains
    ...
    impure subroutine s_compute_rhs(q_cons_vf, q_T_sf, q_prim_vf, bc_type, rhs_vf, pb_in, rhs_pb, mv_in, rhs_mv, t_step, &
        ...
        if (bubbles_lagrange) then
            if (lag_params%initial_corrector .and. .not. adap_dt .and. stage == 1) then
                call s_initial_pressure_correction(q_prim_qp%vf(1:sys_size), bc_type)
            end if
        ...
        if (run_time_info .or. probe_wrt .or. ib .or. bubbles_lagrange .or. hifu) then
            if (.not. igr) then
                $:GPU_PARALLEL_LOOP(private='[i, j, k, l]', collapse=4)
                do i = 1, sys_size
                    do l = idwbuff(3)%beg, idwbuff(3)%end
