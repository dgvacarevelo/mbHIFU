...
contains
    ...
    impure subroutine s_compute_bubble_EE_source(q_cons_vf, q_prim_vf, rhs_vf, divu_in)
        ...
        real(wp) :: dmMass_v, dmMass_n, dmBeta_c, dmBeta_t, dmCson, dmshell, dmRbuck, dmRrupt, dmQvis, dmQth, dmRmean, dmKe, &
             & dmVolmean
        real(wp) :: dmNoise_constant, dmLambda_c, dmdk, dmLoc, dmTime, dmInt, dmA, dmRcell, dmQac
                        ...
                        if (alf < small_alf) then
                            ...
                            if (adap_dt) then
                                ...
                                call s_advance_step(myRho, myP, myR, myV, R0(q), pb_local, pbdot, alf, n_tait, B_tait, &
                                                    ...
                                                    & dmBeta_c, dmBeta_t, dmCson, dmInt, dmshell, dmRbuck, dmRrupt, dmRcell, &
                                                    & dmNoise_constant, dmLambda_c, dmdk, dmLoc, dmTime, dmA, dmQvis, dmQth, &
                                                    & dmQac, dmKe, dmRmean, dmVolmean, adap_dt_stop)
                                ...
                                rddot = f_rddot(myRho, myP, myR, myV, R0(q), pb_local, pbdot, alf, n_tait, B_tait, bub_adv_src(j, &
                                                & k, l), divu_in%sf(j, k, l), dmCson, dmInt, dmshell, dmRbuck, dmRcell)
