...
def _load():
    ...
    for f in range(1, NF + 1):
        ...
        _r(f"{px}absCoef", REAL, {"hifu"}, math=r"\f$absCoef_k\f$")
        _r(f"{px}tdiff", REAL, {"hifu"}, math=r"\f$tdiff_k\f$")
        _r(f"{px}rho_cp", REAL, {"hifu"}, math=r"\f$rho_cp_k\f$")
    ...
    # acoustic_bc_params
    for a in ["iwave", "ncycles"]:
        _r(f"acoustic_bc_params%{a}", INT, {"hifu"})
    for a in ["Pbase", "rho", "cson", "Pamp", "freq", "focLen", "focCal", "apert"]:
        _r(f"acoustic_bc_params%{a}", REAL, {"hifu"})
...
TYPED_DECLS: dict[str, tuple] = {
    ...
    "acoustic_bc_params": ("type(acoustic_bc_parameters)", None, True, "Acoustic source from BC == -20's parameters"),
    "hifu_params": ("type(hifu_parameters)", None, True, "HIFU parameters"),
}
